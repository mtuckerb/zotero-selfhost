...
- `Z_CONFIG::$BASE_URI` is pinned to `http://zotero.org/` and is intentionally not derived from `baseUri`. It is the RDF identifier namespace for object URIs (related items, Atom feed ids), not a network location, and every Zotero client emits `http://zotero.org/...` URIs. Repointing it at your own hostname makes `Zotero_URI::getURIObject()` reject those URIs, so every related-item write fails with HTTP 500 and Related -> Add Related Item -> Add silently does nothing in the web library. Fetchable URLs come from `$API_BASE_URI` / `$WWW_BASE_URI`, which do follow `baseUri`.
